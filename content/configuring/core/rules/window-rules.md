...
<!-- TODO: Table arguments should be { x = int, y = int }, https://github.com/hyprwm/Hyprland/discussions/16339. When (i hope it is "when" and not "if") this is fixed, section below should be edited -->

Position and size arguments take a Lua table or a string.
Value can be a number or an [expression](#expressions).
The string argument is split at the first whitespace, so expressions should not contain spaces to be parsed correctly.
...
-- Move window center to the cursor
...
A single value, a table without exactly two elements, the `{ x = ..., y = ... }` form and any separator other than whitespace (`"800,600"`, `"800x600"`) are config errors.
